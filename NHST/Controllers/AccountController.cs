...
namespace NHST.Controllers
{
    public class AccountController
    {
        ...
        public static string UpdateFeeShipVN(int ID, int FeeShip)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_Account.Where(ac => ac.ID == ID).FirstOrDefault();
        ...
        public static List<tbl_Account> GetAll()
        {
            ...
            {
                List<tbl_Account> las = new List<tbl_Account>();
                las = dbe.tbl_Account.Where(a => a.Status == 2).OrderByDescending(a => a.ID).ToList();
                return las;
            }
        }
        ...
        public static List<View_UserList> GetListUserBySQL_Thien(string searchname, int pageIndex, int pageSize)
        {
            ...
            while (reader.Read())
            {
                ...
                if (reader["FeeShip"] != DBNull.Value)
                    entity.FeeShip = reader["FeeShip"].ToString().ToInt(0);
