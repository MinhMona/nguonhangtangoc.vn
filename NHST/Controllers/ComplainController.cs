...
namespace NHST.Controllers
{
    ...
    {
        #region CRUD
        public static string Insert(int UID, int OrderID, string Amount, string IMG, string ComplainText, int Status, DateTime CreatedDate, string CreatedBy, int TypeComplain, int DathangID)
        {
        ...
        public static string UpdateDathangID(int ID, int DathangID)
        {
            using (var dbe = new NHSTEntities())
            {
                var c = dbe.tbl_Complain.Where(p => p.ID == ID).FirstOrDefault();
        ...
        public static int GetTotal(string s, string fd, string td, int Status, int dathangID, int cskhID, int role, int staff)
        {
            var sql = @"select Total=Count(*) ";
            sql += " from tbl_Complain as cp ";
            sql += " left outer join tbl_MainOder as mo ON mo.ID = cp.OrderID ";
        ...
        public static List<ListComplain> GetAllBySQL(string s, int pageIndex, int pageSize, string fd, string td, int Status, int dathangID, int cskhID, int role, int staff)
        {
            var sql = @"select cp.*, dathang.Username as DathangName, cp.DathangID as DathangID, cskh.Username as CskhName ";
            sql += " from tbl_Complain as cp ";
            sql += " left outer join tbl_MainOder as mo ON mo.ID = cp.OrderID ";
            sql += " left outer join tbl_Account as dathang ON mo.DathangID = dathang.ID ";
            sql += " left outer join  tbl_Account as cskh ON mo.CSID = cskh.ID ";
            ...
            if (dathangID > 0)
            {
                sql += " AND cp.DathangID=" + dathangID + " ";
            }
            if (cskhID > 0)
            {
                sql += " AND cskh.ID=" + cskhID + " ";
