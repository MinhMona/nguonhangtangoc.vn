...
namespace NHST.Controllers
{
    public class MainOrderController
    {
        ...
        public static double GetTotalPriceCustom(string PriceType, string searchtext, int Type, int orderType, int PTVC, string fd, string td, int nvdh, int nvkd, int nvcs)
        {
            ...
            sql += "Where mo.IsHidden = 0 and mo.Status > 1 and  mo.OrderType = '" + orderType + "'";
            if (nvdh > 0)
            {
        ...
        public static double GetTotalPriceDebt(string PriceType, string Username, int RoleID, int StaffID)
        {
            ...
            sql += " WHERE mo.OrderType > 0 ";
            //sql += " AND (mo.Status=5 AND DATEDIFF(day,mo.DateBuy,getdate()) > 20 OR mo.Status=3 AND DATEDIFF(day,mo.DateShipper,getdate()) > 20 OR mo.Status=7 AND DATEDIFF(day,mo.DateToVN,getdate()) > 20)";
            sql += " AND  (convert(float, mo.TotalPriceVND) - convert(float, mo.Deposit)) > 0 AND (mo.Status = 3 OR mo.Status = 5 OR mo.Status = 6 OR mo.Status = 7 OR mo.Status = 8) ";
        ...
        public static List<OrderGetSQL> GetByUserIDInSQLHelperWithFilterOrderList(int RoleID, int StaffID, int OrderType, string searchtext, int Type, string fd, string td, string priceFrom, string priceTo, string st, bool isNotCode, int pageIndex, int pageSize, string mvd, string mdh, int sort, int PTVC, int nvdh, int nvkd, int nvcs)
        {
            ...
            sql += "mo.MainOrderCode,mo.OrderTransactionCode,  mo.Deposit, mo.CreatedDate,ui.Phone, mo.DepostiDate, mo.DateBuy,mo.DateShipper, mo.DateTQ, mo.Site, mo.UID, ";
            sql += " mo.DateVN,mo.PayDate,mo.CompleteDate, mo.Status, mo.OrderType, mo.IsCheckNotiPrice, mo.DateBuyOK, mo.DateToVN, mo.DateToCancel, mo.DateToShip,    ";
